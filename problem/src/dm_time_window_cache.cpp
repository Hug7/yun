...
std::vector<TimeWindow*> NodeTimeWindowCache::get_pick_drop_time_windows(Node* node) {
  auto key = new NodeTimeWindowKey(node->activity_type, node->loc, node->get_orders());
...
std::vector<TimeWindow*> NodeTimeWindowCache::get_other_time_windows(Node* node) {
  auto key = new NodeTimeWindowKey(node->activity_type, node->loc);
