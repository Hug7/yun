...
void LabelsetValueBitset::merge(std::unique_ptr<LabelsetValueBitset> &labelset_value_bitset)
{
...
int Labelset::get_label_ind(const std::string &code)
{
...
LabelsetValue *Labelset::empty_labelset_value()
{
...
std::unique_ptr<LabelsetValueBitset> Labelset::empty_labelset_value_bitset()
{
...
void LabelManager::labelset_add_label(const std::string &code, const std::string &labelset_item)
{
    ...
    if (label == nullptr)
    {
        throw std::runtime_error("Label " + code + " is not exist in LabelManager.labelset_add_label(...)");
    }
