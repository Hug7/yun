...
TimeWindowPlan *TimeWindowInfer::forward_infer_s_s(
    const TimeWindowPlan *pre_node_plan_tw,
    long cost_time,
    long work_time,
    TimeWindow *ori_tw)
{
...
void TimeWindowInfer::forward_infer_s_m(
    const TimeWindowPlan *pre_node_plan_tw,
    long cost_time,
    long work_time,
    ...
    for (const auto &cur_ori_tw : ori_tws)
    {
        if (plan_late_arr < cur_ori_tw->early)
        {
            ...
            if (plan_node_tws.size() == 0 || (plan_node_tws[0]->over_time > 0 || (cur_ori_tw->early - plan_late_arr) < plan_node_tws[0]->wait_time))
            {
                ...
                plan_node_tws.clear();
                plan_node_tws.push_back(forward_infer_s_s(pre_node_plan_tw, cost_time, work_time, cur_ori_tw));
            }
        }
...
std::vector<TimeWindowPlan *> TimeWindowInfer::forward_infer(
    std::vector<TimeWindowPlan *> &pre_node_plan_tws,
    long cost_time,
    long work_time,
    std::vector<TimeWindow *> &ori_tws)
{
...
std::vector<TimeWindow *> TimeWindowUntils::merge_time_windows(std::vector<TimeWindow *> tws)
{
    ...
    for (auto &tw : tws)
    {
        if (res_tws.size() == 0)
        {
            ...
            if ((tw->early - last_tw->late) <= TimeWindowParameter::INTERVAL_SECS_CONTINUOUS_TIME_BUCKET)
            {
                res_tws.pop_back();
