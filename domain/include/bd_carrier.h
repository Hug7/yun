...
public:
    ...
    Carrier(const std::string &code, const std::string &name, const int ind,
            LabelsetValue *labelset_value,
            std::unique_ptr<LabelsetValueBitset> labelset_value_bitset) : Item(code, name, ind),
                                                                          vehicles(),
                                                                          vehicle_map(),
...
public:
    ...
    CarrierManager(Labelset *labelset) : carrier_generate_index(std::make_unique<GenerateIndex>()),
     ...
     * @return 车辆对象指针
     */
    Vehicle *create_vehicle(const std::string &carrier_code,
     ...
     * @return 车辆位图指针
     */
    std::unique_ptr<Bitset> empty_vehicle_bitset();

    /**
     * @brief 创建全为真的车辆位图
     * @return 全为真的车辆位图指针
     */
    std::unique_ptr<Bitset> full_vehicle_bitset();
