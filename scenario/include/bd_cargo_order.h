...
class CargoSubOrder : public Item {
 public:
  ...
  CargoSubOrder(const std::string& code, const std::string& name, const int ind,
                std::vector<long>& dim_vals, LabelsetValue* labelset_value,
                std::unique_ptr<LabelsetValueBitset> labelset_value_bitset, const int qty)
      : Item(code, name, ind),
        dim_vals(dim_vals),
...
class CargoOrder : public Item {
 ...
 public:
  ...
  CargoOrder(const std::string& code, const std::string& name, const int ind,
             const Location* pick_loc, const Location* drop_loc)
      : Item(code, name, ind),
   ...
   * @return 订单明细对象指针
   */
  CargoSubOrder* create_cargo_sub_order(const std::string& code, const std::string& name,
                                        std::vector<long>& dim_vals, LabelsetValue* labelset_value,
                                        std::unique_ptr<LabelsetValueBitset> labelset_value_bitset,
                                        const int qty);
  /**
...
class CargoOrderManager {
 public:
  ...
  CargoOrderManager(const Labelset* labelset, const DimensionManager* dimension_manager)
      : cargo_orders(),
        cargo_order_map(),
        generate_index(std::make_unique<GenerateIndex>()),
        len(0),
   ...
   */
  CargoOrder* create_cargo_order(const std::string& code, const std::string& name,
   ...
   */
  CargoSubOrder* create_cargo_sub_order(const std::string& cargo_order_code,
                                        const std::string& cargo_sub_order_code,
                                        const std::string& cargo_sub_order_name, const int qty);
};
