...
LabelManager* StandardCsvReader::loading_label() {
  ...
  for (int u = 0; u < label_count; u++) {
    ...
    const std::string dim_name =
        label_doc.GetCell<std::string>(LabelSchema::headers[LabelSchema::NAME], u);
  ...
  for (int u = 0; u < label_value_count; u++) {
    ...
    const std::string label_value = label_value_doc.GetCell<std::string>(
        LabelValueSchema::headers[LabelValueSchema::LABEL_VALUE], u);
    Label* label = label_manager->get_label(label_code);
    if (label == nullptr) {
      throw std::runtime_error(std::string(LabelValueSchema::file_name) +
                               " label_code: " + label_code + " not found");
    }
    LabelValue* label_value_ptr = label->add_label_value(label_value);
    if (label_value_ptr == nullptr) {
      throw std::runtime_error(std::string(LabelValueSchema::file_name) +
                               " label_value: " + label_value + " is repetitive");
    }
  }
  ...
  for (int u = 0; u < label_apply_row_count; u++) {
    ...
    const std::string apply_item = label_apply_doc.GetCell<std::string>(
        LabelApplySchema::headers[LabelApplySchema::APPLY_ITEM], u);
    // 校验 label code 是否存在
    if (label_manager->get_label(label_code) == nullptr) {
      throw std::runtime_error(std::string(LabelApplySchema::file_name) +
                               " label code: " + label_code + " not exist");
    }
    // 解析label和实体的映射关系
    if (!LabelSetSupportItem::item_map.count(apply_item)) {
      throw std::runtime_error(std::string(LabelApplySchema::file_name) +
                               " apply item: " + apply_item + " not support");
    }
