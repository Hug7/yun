...
void StandardCsvReader::loading_vehicle(CarrierManager* carrier_manager,
  ...
  for (int u = 0; u < vehicle_count; u++) {
    ...
    const std::string vehicle_model_code = vehicle_doc.GetCell<std::string>(
        VehicleSchema::headers[VehicleSchema::VEHICLE_MODEL_CODE], u);
    const int count = vehicle_doc.GetCell<int>(VehicleSchema::headers[VehicleSchema::COUNT], u);
    const std::string origin_location_code = vehicle_doc.GetCell<std::string>(
        VehicleSchema::headers[VehicleSchema::ORIGIN_LOCATION_CODE], u);
    const std::string destination_location_code = vehicle_doc.GetCell<std::string>(
        VehicleSchema::headers[VehicleSchema::DESTINATION_LOCATION_CODE], u);

    Carrier* carrier = carrier_manager->get_carrier(carrier_code);
    if (carrier == nullptr) {
      throw std::runtime_error("Carrier " + carrier_code + " was not found in " +
                               VehicleSchema::file_name);
    }
