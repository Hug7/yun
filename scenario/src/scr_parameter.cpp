...
Parameter *StandardCsvReader::loading_parameter()
{
    ...
    bool file_exist_flag = FileUtils::file_exists(parameter_file_path, ParameterSchema::file_name, false);
    if (!file_exist_flag)
    {
    ...
    {
        const std::string parameter_code = parameter_doc.GetCell<std::string>(ParameterSchema::headers[ParameterSchema::CODE], u);
        const std::string parameter_value = parameter_doc.GetCell<std::string>(ParameterSchema::headers[ParameterSchema::VALUE], u);
        // todo parse value of parameter
    }
    parameter_doc.Clear();
    spdlog::info("Loading {} is complete. A total of {} pieces of data have been obtained.", ParameterSchema::file_name, row_count);
