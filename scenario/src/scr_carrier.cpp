...
CarrierManager *StandardCsvReader::loading_carrier(LabelManager *label_manager)
{
    ...
    {
        const std::string carrier_code = carrier_doc.GetCell<std::string>(CarrierSchema::headers[CarrierSchema::CODE], u);
        const std::string carrier_name = carrier_doc.GetCell<std::string>(CarrierSchema::headers[CarrierSchema::NAME], u);
    ...
    const std::string carrier_label_value_file_path = this->root_dir + "/" + CarrierLabelValueSchema::file_name;
    ...
    CsvUtils::check_column_exist(carrier_label_value_doc, CarrierLabelValueSchema::headers, CarrierLabelValueSchema::file_name);

    int carrier_label_value_count = carrier_label_value_doc.GetRowCount();
    for (int u = 0; u < carrier_label_value_count; u++)
    {
        const std::string carrier_code = carrier_doc.GetCell<std::string>(CarrierLabelValueSchema::headers[CarrierLabelValueSchema::CARRIER_CODE], u);
        const std::string label_code = carrier_doc.GetCell<std::string>(CarrierLabelValueSchema::headers[CarrierLabelValueSchema::LABEL_CODE], u);
        const std::string label_value = carrier_doc.GetCell<std::string>(CarrierLabelValueSchema::headers[CarrierLabelValueSchema::LABEL_VALUE], u);

        Carrier *carrier = carrier_manager->get_carrier(carrier_code);
        if (carrier == nullptr)
        {
            throw std::runtime_error("Carrier " + carrier_code + " was not found in " + CarrierLabelValueSchema::file_name);
        }

        Label *label = label_manager->get_label(label_code);
        if (label == nullptr)
        {
            throw std::runtime_error("Label " + label_code + " was not found in " + CarrierLabelValueSchema::file_name);
        }

        LabelValue *label_value_ptr = label->get_label_value(label_value);
        if (label_value_ptr == nullptr)
        {
            throw std::runtime_error("Label value " + label_value + " was not found for Label " + label_code + " in " + CarrierLabelValueSchema::file_name);
        }

        int label_ind4labelset = carrier_manager->labelset->get_label_ind(label_code);
        if (label_ind4labelset == -1)
        {
            throw std::runtime_error("Label " + label_code + " was not found in labelset of carrier" + carrier_code);
        }
