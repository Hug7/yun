...
LocationManager::LocationManager(Labelset* labelset)
  ...
  this->create_location(LocationParameter::DEFAULT_LOCATION_CODE,
                        LocationParameter::DEFAULT_LOCATION_CODE, 0, 0);
}
...
Location* LocationManager::create_location(const std::string& code, const std::string& name,
  ...
  if (this->location_map.count(code)) {
    ...
    Location* location = new Location(code, name, ind, lat, lng, work_plan, labelset_value,
                                      std::move(labelset_value_bitset));
