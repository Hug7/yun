                                                  ...
                                                  LocationManager *location_manager)
{
    ...
    const std::string location_available_vehicle_file_path = this->root_dir + "/" + LocationAvailableVehicleSchema::file_name;
    FileUtils::file_exists(location_available_vehicle_file_path, LocationAvailableVehicleSchema::file_name);
    rapidcsv::Document location_available_vehicle_doc(location_available_vehicle_file_path);
    CsvUtils::check_column_exist(location_available_vehicle_doc, LocationAvailableVehicleSchema::headers, LocationAvailableVehicleSchema::file_name);
    ...
    {
        const std::string location_code = location_available_vehicle_doc.GetCell<std::string>(LocationAvailableVehicleSchema::headers[LocationAvailableVehicleSchema::LOCATION_CODE], u);
        const std::string carrier_code = location_available_vehicle_doc.GetCell<std::string>(LocationAvailableVehicleSchema::headers[LocationAvailableVehicleSchema::CARRIER_CODE], u);
        const std::string vehicle_model_code = location_available_vehicle_doc.GetCell<std::string>(LocationAvailableVehicleSchema::headers[LocationAvailableVehicleSchema::VEHICLE_MODEL_CODE], u);
        Location *location = location_manager->get_location(location_code);
        if (location == nullptr)
        {
            throw std::runtime_error("Location " + location_code + " was not found in " + LocationAvailableVehicleSchema::file_name);
        }
        Carrier *carrier = carrier_manager->get_carrier(carrier_code);
        if (carrier == nullptr)
        {
            throw std::runtime_error("Carrier " + carrier_code + " was not found in " + LocationAvailableVehicleSchema::file_name);
        }
        Vehicle *vehicle = carrier->get_vehicle(vehicle_model_code);
        if (vehicle == nullptr)
        {
            throw std::runtime_error("Vehicle model " + vehicle_model_code + " was not found in " + LocationAvailableVehicleSchema::file_name);
        }
