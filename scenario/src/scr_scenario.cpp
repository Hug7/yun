...
Scenario *StandardCsvReader::loading_scenario()
{
    ...
    // 读取车型管理器
    VehicleModelManager *vehicle_model_manager = this->loading_vehicle_model(dimension_manager, label_manager, dist_matrix_manager);
    // 读取承运商
    ...
    // 读取订单
    CargoOrderManager *cargo_order_manager = this->loading_cargo_order(location_manager, dimension_manager, label_manager);
    // 读取参数
