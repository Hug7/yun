 ...
 */
void parse_time_range_and_update_calendar(const std::string& str_time, Calendar* calendar,
                                          const int day_ind) {
  if (!str_time.empty()) {
    ...
    for (auto& str_time_range : str_time_ranges) {
      ...
      auto time_range = TimeFromatted::hhmmhhmm_parse_time_range(str_time_range);
      // check whether time range is valid: left < right
      if (time_range.first >= time_range.second) {
        throw std::runtime_error("invalid time range: " + str_time_range);
      }
      time_ranges.emplace_back(time_range);
    }

    // todo check time ranges overlap

    calendar->set_one_day_time_ranges(day_ind, time_ranges);
  }
}
...
void StandardCsvReader::loading_work_plan(LocationManager* location_manager,
  ...
  CsvUtils::check_column_exist(work_fixed_doc, WorkFixedSchema::headers,
                               WorkFixedSchema::file_name);

  size_t work_plan_count = work_fixed_doc.GetRowCount();
  for (size_t u = 0; u < work_plan_count; u++) {
    const std::string location_code = work_fixed_doc.GetCell<std::string>(
        WorkFixedSchema::headers[WorkFixedSchema::LOCATION_CODE], u);
    const int fixed_pick_time =
        work_fixed_doc.GetCell<int>(WorkFixedSchema::headers[WorkFixedSchema::FIXED_PICK_TIME], u);
    const int fixed_drop_time =
        work_fixed_doc.GetCell<int>(WorkFixedSchema::headers[WorkFixedSchema::FIXED_DROP_TIME], u);
  ...
  CsvUtils::check_column_exist(work_effect_doc, WorkEffectSchema::headers,
                               WorkEffectSchema::file_name);

  size_t work_effect_count = work_effect_doc.GetRowCount();
  for (size_t u = 0; u < work_effect_count; u++) {
    ...
    const double per_hour_process_quantity = work_effect_doc.GetCell<double>(
        WorkEffectSchema::headers[WorkEffectSchema::PER_HOUR_PROCESS_QUANTITY], u);
    Location* location = location_manager->get_location(location_code);
    if (location == nullptr) {
      throw std::runtime_error("Location " + location_code + " was not found in " +
                               WorkEffectSchema::file_name);
  ...
  CsvUtils::check_column_exist(work_calendar_daily_doc, WorkCalendarDailySchema::headers,
                               WorkCalendarDailySchema::file_name);

  size_t work_calendar_daily_count = work_calendar_daily_doc.GetRowCount();
  for (size_t u = 0; u < work_calendar_daily_count; u++) {
    const std::string location_code = work_calendar_daily_doc.GetCell<std::string>(
        WorkCalendarDailySchema::headers[WorkCalendarDailySchema::LOCATION_CODE], u);
    const std::string calendar_type = work_calendar_daily_doc.GetCell<std::string>(
        WorkCalendarDailySchema::headers[WorkCalendarDailySchema::CALENDAR_TYPE], u);
    const std::string daily = work_effect_doc.GetCell<std::string>(
        WorkCalendarDailySchema::headers[WorkCalendarDailySchema::DAILY], u);
    ...
    if (calendar_type_it == CalendarTypeMap.end()) {
      throw std::runtime_error("CalendarType " + calendar_type + " was not support in " +
                               WorkCalendarDailySchema::file_name);
    }
  ...
  const std::string work_calendar_weekly_file_path =
      this->root_dir + "/" + WorkCalendarWeeklySchema::file_name;
  ...
  CsvUtils::check_column_exist(work_calendar_weekly_doc, WorkCalendarWeeklySchema::headers,
                               WorkCalendarWeeklySchema::file_name);
  size_t work_calendar_week_count = work_calendar_weekly_doc.GetRowCount();
  for (size_t u = 0; u < work_calendar_week_count; u++) {
    const std::string location_code = work_calendar_weekly_doc.GetCell<std::string>(
        WorkCalendarWeeklySchema::headers[WorkCalendarWeeklySchema::LOCATION_CODE], u);
    const std::string calendar_type = work_calendar_weekly_doc.GetCell<std::string>(
        WorkCalendarWeeklySchema::headers[WorkCalendarWeeklySchema::CALENDAR_TYPE], u);
    const std::string monday = work_calendar_weekly_doc.GetCell<std::string>(
        WorkCalendarWeeklySchema::headers[WorkCalendarWeeklySchema::MONDAY], u);
    const std::string tuesday = work_calendar_weekly_doc.GetCell<std::string>(
        WorkCalendarWeeklySchema::headers[WorkCalendarWeeklySchema::TUESDAY], u);
    const std::string wednesday = work_calendar_weekly_doc.GetCell<std::string>(
        WorkCalendarWeeklySchema::headers[WorkCalendarWeeklySchema::WEDNESDAY], u);
    const std::string thursday = work_calendar_weekly_doc.GetCell<std::string>(
        WorkCalendarWeeklySchema::headers[WorkCalendarWeeklySchema::THURSDAY], u);
    const std::string friday = work_calendar_weekly_doc.GetCell<std::string>(
        WorkCalendarWeeklySchema::headers[WorkCalendarWeeklySchema::FRIDAY], u);
    const std::string saturday = work_calendar_weekly_doc.GetCell<std::string>(
        WorkCalendarWeeklySchema::headers[WorkCalendarWeeklySchema::SATURDAY], u);
    const std::string sunday = work_calendar_weekly_doc.GetCell<std::string>(
        WorkCalendarWeeklySchema::headers[WorkCalendarWeeklySchema::SUNDAY], u);
    Location* location = location_manager->get_location(location_code);
    if (location == nullptr) {
      throw std::runtime_error("Location " + location_code + " was not found in " +
                               WorkCalendarWeeklySchema::file_name);
