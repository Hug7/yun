...
Vehicle* CarrierManager::create_vehicle(const std::string& carrier_code,
                                        const VehicleModel* vehicle_model, const int count,
  ...
  Vehicle* vehicle = new Vehicle(carrier, this->vehicle_generate_index->next(), vehicle_model,
                                 count, orig_loc, dest_loc);
...
Bitset::UPtr CarrierManager::empty_vehicle_bitset() {
  return std::make_unique<Bitset>(this->vehicle_len);
}

Bitset::UPtr CarrierManager::full_vehicle_bitset() {
  auto bitset = std::make_unique<Bitset>(this->vehicle_len);
  bitset->set_all();
  return bitset;
}
