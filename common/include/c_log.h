...
struct LoggerConfig {
  ...
  LoggerConfig()
      : log_level("info"), log_file(""), prefix(""), pattern("[%Y-%m-%d %H:%M:%S] [%l] %v") {}
};

void init_logger(std::string log_level, std::string log_file, std::string prefix,
                 std::string pattern) {
  try {
    // 控制台接收器
...
void init_logger(LoggerConfig& logger_config) {
  init_logger(logger_config.log_level, logger_config.log_file, logger_config.prefix,
