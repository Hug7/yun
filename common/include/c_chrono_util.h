...
inline bool valid_time(const Time& t) {
  return t.hour >= 0 && t.hour < 24 && t.minute >= 0 && t.minute < 60 && t.second >= 0 &&
         t.second < 60 && t.nanosecond >= 0 && t.nanosecond < 1000000000;
}
...
inline int day_of_week(const DateTime& dt) { return day_of_week(dt.date); }
...
inline int64_t days_from_civil(const Date& d) { return days_from_civil(d.year, d.month, d.day); }
...
inline int64_t to_timestamp(const DateTime& dt) {
  ...
  int64_t sec_of_day =
      (int64_t)dt.time.hour * 3600 + (int64_t)dt.time.minute * 60 + (int64_t)dt.time.second;
...
inline bool parse_format(const char* s, const char* fmt, DateTime& out) {
  ...
  static const char* months_abbr[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
                                      "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
  static const char* months_full[] = {"January",   "February", "March",    "April",
      ...
      switch (*f) {
        ...
        case 'z': {
          if (*p == 'Z' || *p == 'z') {
            ...
            if (*p == ':') ++p;  // skip colon in +HH:MM
            if (*p >= '0' && *p <= '9') tz_off += sign * parse_int2(p);
        ...
        case 'b': {
          ...
          for (int i = 0; i < 12; ++i) {
            ...
            if (strncasecmp(p, months_abbr[i], len) == 0 &&
                (p[len] == ' ' || p[len] == '-' || p[len] == ',' || p[len] == 0)) {
              M = i + 1;
              p += len;
              found = true;
              break;
  ...
  if (!valid_date(out.date) || !valid_time(out.time)) return false;
...
inline int nth_weekday_of_month(int year, int month, int weekday, int n) {
  ...
  if (day > dim) day -= 7;  // "last" overflows to previous week
  return day;
}
...
inline int effective_utc_offset(const TzEntry* tz, int64_t utc_ts) {
  ...
  auto is_europe = [&]() -> bool {
    return strncmp(name, "Europe/", 7) == 0 || strncmp(name, "Atlantic/", 9) == 0 ||
           strcmp(name, "Asia/Nicosia") == 0 || strcmp(name, "Asia/Jerusalem") == 0;
  };

  // --- Australia: 1st Sun Oct -> 1st Sun Apr ---
  auto is_australia = [&]() -> bool { return strncmp(name, "Australia/", 10) == 0; };

  // --- Middle East (approximate): last Fri Mar -> last Fri Oct ---
  auto is_middle_east = [&]() -> bool {
    return strcmp(name, "Asia/Amman") == 0 || strcmp(name, "Asia/Beirut") == 0 ||
           strcmp(name, "Asia/Damascus") == 0 || strcmp(name, "Africa/Casablanca") == 0;
  };

  // --- Iran: Mar 21/22 -> Sep 21/22 ---
  auto is_iran = [&]() -> bool { return strcmp(name, "Asia/Tehran") == 0; };

  // --- Chile: varies; approximate as Oct -> Mar (southern hemisphere) ---
  auto is_chile = [&]() -> bool {
    return strcmp(name, "America/Santiago") == 0 || strcmp(name, "Pacific/Easter") == 0;
  };

  // --- Pacific: Norfolk ---
  auto is_norfolk = [&]() -> bool { return strcmp(name, "Pacific/Norfolk") == 0; };

  // --- Chatham ---
  auto is_chatham = [&]() -> bool { return strcmp(name, "Pacific/Chatham") == 0; };
...
inline DateTime to_utc(const DateTime& dt) { return to_timezone(dt, "UTC"); }
...
inline void _apply_default_timezone(DateTime& dt) {
  if (!_g_default_tz_name[0]) return;
  if (dt.tz_name != nullptr) return;  // already has explicit timezone
...
inline int64_t to_unix(const DateTime& dt) { return to_timestamp(dt); }
...
inline bool is_valid_timezone(const char* tz_spec) {
  if (!tz_spec || !*tz_spec) return false;
  if (find_tz_entry(tz_spec)) return true;
  int off = 0;
  const char* nm = nullptr;
...
inline const TzEntry* get_timezone_info(const char* tz_spec) {
  if (!tz_spec || !*tz_spec) return nullptr;
